...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const mainContainer = document.querySelector('.option_list'); // 옵션들이 쌓일 메인 컨테이너
    const totalPriceElement = document.getElementById('total_price'); // 총 상품금액을 표시할 div
    ...
    optionSelect.addEventListener('change', function () {
        ...
        newOptionInner.style.display = 'block'; // 복제된 요소가 보이도록 설정

        // 옵션명 업데이트
        newOptionInner.querySelector('.size_select').textContent = selectedOption; 
        ...
        function updateValues() {
            ...
            document.querySelectorAll('.option_inner').forEach(option => {
                const optionQuantity = parseInt(option.querySelector('#select_list_quantity').value);
                totalQuantity += optionQuantity;
                totalPrice += productPrice * optionQuantity;
            });

            // 총 상품금액 업데이트
            totalPriceElement.textContent = `총 상품금액: ${totalPrice.toLocaleString()}원 (${totalQuantity}개)`;
        }
        ...
        quantityUp.addEventListener('click', function (e) {
            ...
            const optionTotalPrice = productPrice * currentValue;
            selectTotalPriceElement.textContent = `${optionTotalPrice.toLocaleString()}원`;
            updateValues();  // 총 금액 및 수량 갱신
        });
